...
flash:
	$(OPENOCD_PATH)/openocd -f $(OPENOCD_PATH)/$(OPENOCD_CFG) -c init -c halt -c "flash erase_sector wch_riscv 0 last" -c "program $(BDIR)/$(PROJECT).hex" -c wlink_reset_resume -c exit

reset:
	$(OPENOCD_PATH)/openocd -f $(OPENOCD_PATH)/$(OPENOCD_CFG) -c init -c halt -c wlink_reset_resume -c exit

debug:
	$(OPENOCD_PATH)/openocd -f $(OPENOCD_PATH)/$(OPENOCD_CFG) 
